...
const ModalEditSubcategory = (props) => {
  ...
  const FetchEditCategory = async (e) => {
    ...
    setError(null);
    try {
      await axios.put(`${apiUrl}/subcategories/${props.id}`, {name, imageUrl});
      ...
      if (err.response) {
        ...
        setError('Помилка редагування підкатегорії');
      }
    } finally {
      setLoading(false);
    }
      ...
      {loading && <Loader />}
          <Modal show={props.modalEditShow} onHide={props.handleElitCloseModal}>
            <Modal.Header closeButton>
              <Modal.Title>Редагувати підкатегорію</Modal.Title>
            </Modal.Header>
            <Modal.Body>
